...
def subgradient_descent( func, initial_x, maximum_iterations=10, initial_stepsize=1):
    """ 
    Subgradient Descent
    func:               the function to optimize. It is called as "value, gradient = func( x, 1 )
    initial_x:          the starting point, should be a float
    ...
    while True:
        ...
        x = x - (initial_stepsize/math.sqrt(iterations)) * gradient 
...
def adagrad( func, initial_x, maximum_iterations=10, initial_stepsize=1, initial_sum_of_squares=1e-3):
    """ 
    adagrad
    func:                   the function to optimize. It is called as "value, gradient = func( x, 1 )
    initial_x:              the starting point, should be a float
    ...
    start_time = time.time()
    iterations = 0
    inital_sum_square = np.array(x.shape)
    inital_sum_square.fill(initial_sum_of_squares)
    sum_gradient= np.zeros(initial_x.shape)
    # subgradient updates
    while True:
        ...
        temp = np.multiply(gradient, gradient)
        sum_gradient = sum_gradient + temp
        eta = initial_stepsize / (np.sqrt(initial_sum_of_squares + sum_gradient))
