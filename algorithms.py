...
def bfgs( func, initial_x, initial_inv_h, eps=1e-5, maximum_iterations=65536, linesearch=bisection, *linesearch_args  ):
    ...
    *linesearch_args:   the extra arguments of linesearch routine
    ...
    while True:
        ...
        if np.linalg.norm(gradient)**2 <= eps:   # Stopping criteria
            break
        
        Yt=np.subtract(gradient,old_gradient)    # Calculating Yt and St
        St=np.subtract(x,old_x)
        if(Yt.T*St > 1e-9):
            Pt=np.linalg.inv(Yt.T*St)
            inv_h_t=(np.identity(len(St))-Pt.item(0)*St*Yt.T)*inv_h*(np.identity(len(St))-Pt.item(0)*Yt*St.T)+Pt.item(0)*St*St.T
        else:
            inv_h_t=inv_h
            
        direction = -inv_h_t * gradient
        
        t = linesearch( func, x, direction )
        
        old_x=x                                #Updating x and gradient
